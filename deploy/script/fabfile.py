...
def send_confile(confile, service_name=None):
    with settings(hide('running', 'stdout'), warn_only=True):
        ...
        if not service_name or len(service_name) <= 1 or service_name == ".*":
            print(red("error service name"))
            return
        put('../conf/unichain_confiles/key/' + confile, 'key')
        put('../conf/unichain_confiles/param/' + confile, 'param')
        put('../conf/unichain_confiles/server/' + confile, 'server')
        run('mv key ~/.{}'.format("unichain_key"))
        run('mv param ~/.{}'.format("unichain_param"))
        run('mv server ~/.{}'.format("unichain_server"))
...
def update_unichain_from_archive(service_name=None, setup_name=None):
    ...
    with settings(warn_only=True):
        ...
        put('unichain-archive.tar.gz' , '~/unichain-archive.tar.gz')
...
@parallel
@function_tips()
def detect_rethinkdb(index, all_node_number,db="bigchain", port=28015):
    with settings(hide('warnings', 'running', 'stdout'), warn_only=True):
        hostname = sudo("hostname")
        ## 判断rethinkdb进程数
        rethink_number = len(public_hosts)
        process_num = int(run('ps -aux|grep -E "/usr/bin/rethinkdb"|grep -v grep|wc -l'))
        print(blue("rethinkdb 集群个数： {}".format(all_node_number)))
        if process_num < 3:
...
@task
@function_tips()
def detect_unichain(all_node_number,service_name=None):
    with settings(hide('warnings', 'running', 'stdout'), warn_only=True):
        hostname = sudo("hostname")
        if not service_name:
            service_name = _service_name
        ...
        process_num=int(run('ps -aux|grep -E "/usr/local/bin/{} -y start|SCREEN -d -m {} -y start"|grep -v grep|wc -l'
                        .format(service_name, service_name)))
        print(blue("unichain_number 集群个数： {}".format(all_node_number)))
...
@task
@function_tips()
def detect_unichain_api(all_node_number, service_name=None):
    with settings(hide('warnings', 'running', 'stdout'), warn_only=True):
        hostname = sudo("hostname")
        if not service_name:
            service_name = _service_name
        ...
        unichain_conf_str=run('cat ~/.unichain_server')
        ...
        try:
            ...
            if server_url.startswith("0.0.0.0"):
                ...
                if not api_detect_res:
                    ...
                    get('~/.unichain_key', '../report/conf/{}'.format(hostname), use_sudo=True)
            else:
                print(red("{} 节点请求不到，请及时处理！！！".format(hostname)))
...
def bak_unichain_conf(base, service_name=None):
    with settings(hide('running', 'stdout'), warn_only=True):
        if not service_name:
            service_name = _service_name
        get('~/.unichain_key', '{}/unichain/unichain_{}_{}'.format(base, env.user, env.host),
            use_sudo=True)
...
def modify_node_confile():
    ...
    with settings(warn_only=True):
        user = sudo("echo $HOME")
        filepath = user + '/.unichain_key'
        get(filepath, "./unichain_conf")
        sudo('cp ~/.unichain_key ~/.unichain_key_bak')
        local('python3 modify_node_confiles.py')
        put('unichain_conf', "~/.unichain_key")


#  修改节点配置文件
@task
def update_unichain_config(confile):
    """发送unichain配置文件"""
    with settings(warn_only=True):
        put('../conf/unichain_confiles/param/' + confile, 'param')
        put('../conf/unichain_confiles/server/' + confile, 'server')
